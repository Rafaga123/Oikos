...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const registerMetrics = document.getElementById("register-password-metrics");
  ...
  const validators = {
    0: () => {
      ...
      if (!nameRegex.test(nombre1))
        return "Ingrese un primer nombre válido (mín. 2 letras)";
      if (nombre2 && !nameRegex.test(nombre2))
        return "Segundo nombre no válido";
      if (!nameRegex.test(apellido1))
        return "Ingrese un primer apellido válido (mín. 2 letras)";
      if (apellido2 && !nameRegex.test(apellido2))
    ...
    1: () => {
      ...
      const maxAdultBirthDate = getAdultMaxDate();
      if (birthDate > maxAdultBirthDate)
        return "Debes ser mayor de 18 años para registrarte";
      if (!phoneRegex.test(telefono))
        return "El teléfono debe iniciar con + y tener entre 8 y 15 dígitos";
      const matchedDial = countryDialCodes.find((c) =>
        telefono.startsWith(c.dial),
      );
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      segundo_apellido:
        document.getElementById("apellido2").value.trim() || null,
    ...
    try {
      ...
      if (!resp.ok) {
        const message =
          data?.error || "No se pudo registrar. Intente nuevamente.";
        setError(message);
        if (window.Swal) {
          Swal.fire({
            icon: "error",
  ...
  function applyBirthDateLimit() {
    if (!birthInput) return;
    const maxDate = getAdultMaxDate();
    birthInput.max = formatDateInput(maxDate);
  }

  function refreshRegisterMetrics() {
    const pass = document.getElementById("password")?.value || "";
    const conf = document.getElementById("confirmar")?.value || "";
    updatePasswordMetrics(pass, conf, registerMetrics);
  }
  ...
  function applyDialToPhone(dial) {
    ...
    const existing = countryDialCodes.find((c) =>
      normalized.startsWith(c.dial),
    );
    const rest = existing
      ? normalized.slice(existing.dial.length)
      : normalized.replace(/^\+?/, "");
